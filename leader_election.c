...
int main(int argc, char* argv[]) {
    ...
    if (rank != 0) {
        ...
        printf("PROC-%d generated a token of %d\n", rank, token);

        // Determine new president and vice
        int current_value = token % 100;
        int first_value = positions[0] % 100;
        int second_value = positions[1] % 100;
        if (token % 2 == 0 && current_value > first_value) {
            positions[0] = token;
        } else if (token % 2 != 0 && current_value > second_value) {
            positions[1] = token;
        }
